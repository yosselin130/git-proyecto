...
CREATE OR REPLACE FUNCTION P_S01PPRY_3(text)
  RETURNS text AS $$
DECLARE
   --PROCEDIMENTO QUE AUDITA ASIGNACION DE PROYECTO REQUISITO INTEGRANTE 
   p_cData     ALIAS FOR $1;
   --PARÁMETROS CABECERA
...
BEGIN
   ...
   ELSIF NOT p_cEstado IN ('A', 'F', 'X') THEN   
      RETURN '{"ERROR": "ESTADO DEL PROYECTO NO PERMITE AUDITAR"}';
   END IF;
     -- Valida REQUISITO
   ...
   END IF;
      ...
      IF p_cEstado = 'X' THEN
         RETURN '{"ERROR": "PROYECTO YA FUE ANULADO, NO SE PUEDE AUDITAR"}';
      END IF;
   UPDATE H02PPRY SET cEstado = 'A', cNroDni = p_cNroDni, tModifi = NOW() WHERE cCodigo = p_cCodigo;
   RETURN '{"OK": "OK"}';
